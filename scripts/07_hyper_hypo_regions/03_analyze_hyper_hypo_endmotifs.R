...
# Optional: used only for the combined two-panel SeqLogo figure.
...
# ============================================================
# 2B. Helper functions for corrected weighted SeqLogo
# ============================================================

select_top30_volcano_motifs <- function(motif_tests, direction_name, top_n = 30) {
  if (!all(c("motif", "mean_log2FC_hypo_vs_hyper", "p_adj_BH") %in% colnames(motif_tests))) {
    stop("motif_tests must contain: motif, mean_log2FC_hypo_vs_hyper, p_adj_BH")
  }

  motif_tests_clean <- motif_tests %>%
    mutate(
      motif = str_to_upper(as.character(motif)),
      mean_log2FC_hypo_vs_hyper = as.numeric(mean_log2FC_hypo_vs_hyper),
      p_adj_BH = as.numeric(p_adj_BH),
      neg_log10_padj_for_logo = -log10(p_adj_BH + 1e-300),
      logo_weight = abs(mean_log2FC_hypo_vs_hyper) * neg_log10_padj_for_logo
    ) %>%
    filter(
      str_detect(motif, "^[ACGT]{4}$"),
      !is.na(mean_log2FC_hypo_vs_hyper),
      !is.na(p_adj_BH),
      !is.na(logo_weight),
      logo_weight > 0
    )

  if (direction_name == "hyper") {
    selected <- motif_tests_clean %>%
      filter(
        p_adj_BH < padj_cutoff,
        mean_log2FC_hypo_vs_hyper <= -log2fc_cutoff
      ) %>%
      arrange(desc(logo_weight), p_adj_BH, mean_log2FC_hypo_vs_hyper)
  } else if (direction_name == "hypo") {
    selected <- motif_tests_clean %>%
      filter(
        p_adj_BH < padj_cutoff,
        mean_log2FC_hypo_vs_hyper >= log2fc_cutoff
      ) %>%
      arrange(desc(logo_weight), p_adj_BH, desc(mean_log2FC_hypo_vs_hyper))
  } else {
    stop("direction_name must be either 'hyper' or 'hypo'.")
  }

  if (nrow(selected) < top_n) {
    warning(
      "Only ", nrow(selected), " ", direction_name,
      "-enriched motifs passed padj < ", padj_cutoff,
      " and |log2FC| >= ", log2fc_cutoff,
      ". Using all available motifs for the logo."
    )
  }

  selected %>% slice_head(n = top_n)
}

make_weighted_pwm_from_top_motifs <- function(top_motif_df) {
  if (is.null(top_motif_df) || nrow(top_motif_df) == 0) {
    stop("No motifs available to build weighted PWM.")
  }

  if (!"logo_weight" %in% colnames(top_motif_df)) {
    top_motif_df <- top_motif_df %>%
      mutate(
        logo_weight = abs(as.numeric(mean_log2FC_hypo_vs_hyper)) *
          (-log10(as.numeric(p_adj_BH) + 1e-300))
      )
  }

  bases <- c("A", "C", "G", "T")

  motif_position_contributions <- top_motif_df %>%
    transmute(
      motif = str_to_upper(as.character(motif)),
      logo_weight = as.numeric(logo_weight),

      # Explicit fixed-position parsing.
      # This is the correction that prevents position 4 from being lost or misordered.
      pos1 = str_sub(motif, 1, 1),
      pos2 = str_sub(motif, 2, 2),
      pos3 = str_sub(motif, 3, 3),
      pos4 = str_sub(motif, 4, 4)
    ) %>%
    ...
    mutate(
      position = as.integer(str_remove(position, "pos")),
      base = factor(base, levels = bases)
    ) %>%
    filter(
      position %in% 1:4,
      base %in% bases,
  ...
  if (nrow(motif_position_contributions) == 0) {
    stop("No valid motif-position entries after parsing.")
  }

  probability_table <- motif_position_contributions %>%
    group_by(position, base) %>%
    summarise(
      total_weight = sum(logo_weight, na.rm = TRUE),
      .groups = "drop"
    ) %>%
    complete(
      position = 1:4,
    ...
    group_by(position) %>%
    mutate(
      position_total_weight = sum(total_weight, na.rm = TRUE),
      probability = ifelse(
        position_total_weight > 0,
        total_weight / position_total_weight,
        0
      )
    ) %>%
    ungroup() %>%
  ...
  for (i in seq_len(nrow(probability_table))) {
    b <- as.character(probability_table$base[i])
    pos <- as.character(probability_table$position[i])
    pwm[b, pos] <- probability_table$probability[i]
  }

  col_sums <- colSums(pwm)
  if (any(abs(col_sums - 1) > 1e-6)) {
    warning(
      "Some SeqLogo PWM columns do not sum to 1: ",
  ...
  list(
    ...
    motif_position_contributions = motif_position_contributions
  )
}

plot_weighted_seqlogo <- function(pwm, plot_title, plot_subtitle, output_prefix) {
  p <- ggseqlogo(
    pwm,
    method = "prob",
    seq_type = "dna"
    ...
    theme(
      plot.title = element_text(face = "bold", hjust = 0.5, family = plot_family),
      plot.subtitle = element_text(hjust = 0.5, size = 10.5, family = plot_family),
      axis.title = element_text(color = "black", family = plot_family),
      axis.text = element_text(color = "black", family = plot_family)
    )
  ...
  ggsave(
    file.path(figure_dir, paste0(output_prefix, ".pdf")),
    p,
    width = 6.2,
    height = 4.2,
    bg = "white"
...
# ============================================================
# 12B. Corrected weighted SeqLogo from top 30 volcano motifs
# ============================================================

# Select the top volcano-defined motifs:
#   Hyper-enriched: negative mean_log2FC_hypo_vs_hyper
#   Hypo-enriched:  positive mean_log2FC_hypo_vs_hyper
#
# Weighting:
#   logo_weight = |mean_log2FC_hypo_vs_hyper| × -log10(BH-adjusted p-value)
#
# The SeqLogo y-axis is weighted base probability, not raw nucleotide frequency.

top30_hyper_for_seqlogo <- select_top30_volcano_motifs(
  motif_tests = motif_tests,
  direction_name = "hyper",
  top_n = 30
)

top30_hypo_for_seqlogo <- select_top30_volcano_motifs(
  motif_tests = motif_tests,
  direction_name = "hypo",
  top_n = 30
)
...
write_tsv(
  top30_hyper_for_seqlogo,
  file.path(table_dir, "seqlogo_top30_hyper_enriched_motifs_from_volcano.tsv")
)

write_tsv(
  top30_hypo_for_seqlogo,
  file.path(table_dir, "seqlogo_top30_hypo_enriched_motifs_from_volcano.tsv")
)

hyper_seqlogo_obj <- make_weighted_pwm_from_top_motifs(top30_hyper_for_seqlogo)
hypo_seqlogo_obj <- make_weighted_pwm_from_top_motifs(top30_hypo_for_seqlogo)

hyper_seqlogo_pwm <- hyper_seqlogo_obj$pwm
hypo_seqlogo_pwm <- hypo_seqlogo_obj$pwm

cat("\nCorrected top-30 hyper SeqLogo PWM column sums:\n")
print(colSums(hyper_seqlogo_pwm))

cat("\nCorrected top-30 hypo SeqLogo PWM column sums:\n")
print(colSums(hypo_seqlogo_pwm))

write_tsv(
  as.data.frame(hyper_seqlogo_pwm) %>% rownames_to_column("base"),
  file.path(table_dir, "seqlogo_top30_hyper_weighted_pwm.tsv")
)

write_tsv(
  as.data.frame(hypo_seqlogo_pwm) %>% rownames_to_column("base"),
  file.path(table_dir, "seqlogo_top30_hypo_weighted_pwm.tsv")
)

write_tsv(
  hyper_seqlogo_obj$probability_table,
  file.path(table_dir, "seqlogo_top30_hyper_weighted_base_probabilities_long.tsv")
)

write_tsv(
  hypo_seqlogo_obj$probability_table,
  file.path(table_dir, "seqlogo_top30_hypo_weighted_base_probabilities_long.tsv")
)

write_tsv(
  hyper_seqlogo_obj$motif_position_contributions,
  file.path(table_dir, "seqlogo_top30_hyper_motif_position_contributions.tsv")
)

write_tsv(
  hypo_seqlogo_obj$motif_position_contributions,
  file.path(table_dir, "seqlogo_top30_hypo_motif_position_contributions.tsv")
)

p_seqlogo_hyper <- plot_weighted_seqlogo(
  pwm = hyper_seqlogo_pwm,
  plot_title = paste0("Top ", nrow(top30_hyper_for_seqlogo), " hyper-enriched motifs"),
  plot_subtitle = "Weighted by |mean log2FC| × -log10(BH-adjusted p-value)",
  output_prefix = "seqlogo_top30_hyper_enriched_motifs"
)

p_seqlogo_hypo <- plot_weighted_seqlogo(
  pwm = hypo_seqlogo_pwm,
  plot_title = paste0("Top ", nrow(top30_hypo_for_seqlogo), " hypo-enriched motifs"),
  plot_subtitle = "Weighted by |mean log2FC| × -log10(BH-adjusted p-value)",
  output_prefix = "seqlogo_top30_hypo_enriched_motifs"
)

if (has_patchwork) {
  p_seqlogo_combined <- p_seqlogo_hyper / p_seqlogo_hypo

  ggsave(
    file.path(figure_dir, "seqlogo_top30_hyper_hypo_combined.png"),
    p_seqlogo_combined,
    width = 6.2,
    height = 7.8,
    dpi = 300,
  ...
  ggsave(
    file.path(figure_dir, "seqlogo_top30_hyper_hypo_combined.pdf"),
    p_seqlogo_combined,
    width = 6.2,
    height = 7.8,
    bg = "white"
  ...
  warning(
    "Package 'patchwork' is not installed. Separate SeqLogo plots were saved, ",
    "but the combined stacked plot was skipped. Install with: install.packages('patchwork')"
  )
}
...
saveRDS(
  list(
    ...
    hypo_pwm = hypo_pwm,
    diff_pwm = diff_pwm,
    hyper_seqlogo_pwm = hyper_seqlogo_pwm,
    hypo_seqlogo_pwm = hypo_seqlogo_pwm,
    top30_hyper_for_seqlogo = top30_hyper_for_seqlogo,
    top30_hypo_for_seqlogo = top30_hypo_for_seqlogo,
...
cat("\nCorrected SeqLogo outputs:\n")
cat("seqlogo_top30_hyper_enriched_motifs.png\n")
cat("seqlogo_top30_hypo_enriched_motifs.png\n")
cat("seqlogo_top30_hyper_hypo_combined.png if patchwork is installed\n")
