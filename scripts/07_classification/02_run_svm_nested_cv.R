# ============================================================
# SVM nested CV classifier for cfDNA-MS feature sets
#
# This script is based on the original SVM classifier structure:
#   - Same 5 binary tasks
#   - Same nested CV design
#   - Outer 5-fold CV for unbiased test evaluation
#   - Inner 3-fold CV for feature selection + C tuning + threshold selection
...
#
# Only change:
#   The script reads one prepared feature matrix from:
#   results/intermediate/classification/feature_sets/<feature_set>.tsv
#
# Usage:
...
figure_out_dir <- file.path(
  "results/figures/classification/svm",
  feature_set_name
)
...
cat("\n============================================================\n")
cat("SVM nested CV classification\n")
...
train_svm_fixed_C <- function(X_train, y_train, C_value) {

  y_train <- factor(y_train, levels = c("case", "control"))
...
select_features <- function(method_code,
  ...
  if (method_code == "rfe") {
    return(select_rfe(X_train, y_train_binary, n_inner_folds = n_inner_folds_for_rfe))
  }

  if (method_code == "rf_importance") {
    return(select_rf_importance(X_train, y_train_binary))
  }

  if (method_code == "limma_sig") {
    return(select_limma_sig(X_train, y_train_binary, adj_cutoff = 0.05))
  }

  if (method_code == "top50_limma") {
    return(select_top50_limma(X_train, y_train_binary, n_top = n_top))
  }

  if (method_code == "top50_trend") {
    return(select_top50_trend(X_train, group_train_original, n_top = n_top))
  }
...
# ============================================================
# 5. Inner CV: feature selection + C tuning
# ============================================================

tune_C_inner_cv <- function(X_outer_train_raw,
                            y_outer_train,
  ...
  y_outer_train <- factor(y_outer_train, levels = c("case", "control"))
  ...
  for (C_value in C_grid) {
    ...
    pred_C <- bind_rows(inner_predictions)

    if (nrow(pred_C) == 0 ||
        length(unique(pred_C$True_Class)) < 2) {
      ...
      auc_C <- safe_roc_auc(
        response = pred_C$True_Class,
        predictor = pred_C$Predicted_Probability_Case
      )
    }
    ...
    tuning_rows[[length(tuning_rows) + 1]] <- data.frame(
      C = C_value,
      Inner_AUC = auc_C,
      Mean_N_Selected_Features = mean(pred_C$N_Selected_Features, na.rm = TRUE),
      stringsAsFactors = FALSE
    )

    prediction_by_C[[as.character(C_value)]] <- pred_C
  }
  ...
  best_C <- best_row$C[1]
  pred_best <- prediction_by_C[[as.character(best_C)]]
...
run_nested_cv_svm_5methods <- function(data,
                                       y_binary,
                                       group_original,
                                       task_name,
                                       n_outer_folds = 5,
  ...
  y_binary <- factor(y_binary, levels = c("case", "control"))
...
for (task in tasks) {
  ...
  res <- run_nested_cv_svm_5methods(
    data = task_data,
...
write_csv(
  all_fold_metrics,
  file.path(table_out_dir, "svm_5methods_fold_metrics.csv")
)

write_csv(
  all_predictions,
  file.path(table_out_dir, "svm_5methods_outer_fold_predictions.csv")
)

write_csv(
  all_selected_features,
  file.path(table_out_dir, "svm_5methods_selected_features_long.csv")
)

write_csv(
  all_tuning_table,
  file.path(table_out_dir, "svm_5methods_inner_tuning_results.csv")
)
...
summary_table <- all_fold_metrics %>%
  ...
  summarise(
    ...
    Best_C_mean = mean(Best_C, na.rm = TRUE),
...
write_csv(
  summary_table,
  file.path(table_out_dir, "svm_5methods_results_summary.csv")
)
...
write_csv(
  print_summary,
  file.path(table_out_dir, "svm_5methods_results_summary_printable.csv")
)
...
for (task_name in unique(all_predictions$Task)) {
  ...
  ggsave(
    filename = file.path(
      figure_out_dir,
      paste0("ROC_", clean_filename(task_name), "_5methods.png")
    ),
    plot = p_roc,
    width = 7,
    height = 6,
  ...
  ggsave(
    filename = file.path(
      figure_out_dir,
      paste0("ROC_", clean_filename(task_name), "_5methods.pdf")
    ),
    plot = p_roc,
    width = 7,
    height = 6,
...
# ============================================================
# 11. Feature selection frequency
# ============================================================
...
write_csv(
  ...
  file.path(table_out_dir, "svm_5methods_feature_selection_frequency.csv")
)
...
write_csv(
  roc_diagnostics,
  file.path(table_out_dir, "svm_5methods_ROC_diagnostics.csv")
)
